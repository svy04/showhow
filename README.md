...
- **검사 192건이 통과한 상태로 배포합니다.** `node tests/검사_전부.mjs` 로 직접 돌려볼 수 있습니다.
...
showhow 는 클릭 대신 **화면이 얼마나 바뀌었는지** 를 봅니다. 초당 7번 화면을 비교해서, 크게 바뀌기 시작한 뒤 다시 잠잠해지는 순간에 한 장을 담습니다.
...
오래 움직였을수록 더 오래 잠잠해야 담습니다. 그래서 스크롤이나 영상처럼 계속 바뀌는 화면에서 사진이 쌓이지 않습니다.
...
1. **크롬·엣지에서 검사했습니다.** 파이어폭스·사파리에서는 돌려보지 않았습니다. 확인하려면 그 브라우저로 열고 `node tests/검사_촬영.mjs` 와 같은 절차를 손으로 밟으면 됩니다.
2. **화면 고르기 창은 사람이 한 번 눌러야 열립니다.** 브라우저 규칙이라 프로그램이 대신 누를 수 없습니다. 그래서 검사에서도 그 창만 대역으로 두고 나머지 전 과정을 진짜로 돌립니다 (`tests/검사_촬영.mjs` 첫 주석 참조).
3. **링크로 공유하는 기능은 없습니다.** 서버가 없기 때문입니다. 대신 **파일 한 장으로** 를 쓰면 받는 사람이 두 번 눌러 바로 엽니다.
4. **소리와 영상은 담지 않습니다.** 화면 사진과 글만 남습니다.
...
cd showhow
node tests/검사_전체.mjs        # 브라우저 없이 도는 검사 56건

npm i -D playwright             # 브라우저 검사를 돌리려면
npx playwright install msedge
node tests/검사_전부.mjs        # 전부 192건
```

돌리면 이렇게 나옵니다.

```
통과  검사_전체.mjs           56건    0.1초   브라우저 없이 도는 뼈대 검사
통과  검사_양식.mjs            6건    0.1초   양식 저장·불러오기
통과  검사_기능표.mjs          22건    0.1초   상용 제품 기능 대조
통과  검사_브라우저.mjs         77건   11.6초   진짜 브라우저 — 내보내기·보관함·나쁜 상황·규모
통과  검사_촬영.mjs           23건   29.8초   진짜 브라우저 — 자동 촬영 전 과정
