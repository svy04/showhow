...
- **검사 357건이 통과한 상태로 배포합니다.** 그중 227건은 진짜 브라우저에서, 10건은 진짜 화면 공유로 돌립니다.
...
통과    검사_기능표.mjs          31건    0.1초   상용 제품 기능 대조
통과    검사_요청대조.mjs         24건    0.1초   합격 기준 두 축 — 요청 원문·우리 회사 실패
통과    검사_브라우저.mjs        162건   23.5초   진짜 브라우저 — 내보내기·보관함·표시·손질·규격·흐름
통과    검사_촬영.mjs           23건   29.9초   진짜 브라우저 — 자동 촬영 전 과정
통과    검사_인쇄.mjs            8건    3.2초   진짜 브라우저 — PDF 출력
통과    검사_좁은화면.mjs          6건    3.9초   창을 실제로 줄여 본다
통과    검사_제목.mjs           18건     15초   단계 글 자동 작성 — 화면 글자 읽기
통과    검사_진짜공유.mjs         10건   28.6초   대역 없이 진짜 화면 공유 (창이 잠깐 뜬다)
...
`tests/검사_브라우저_규격.js` 는 화면 값이 규격에서 벗어나지 않았는지 잽니다 — 본문 17px, 행간 1.5, 폭 680px, 모서리 세 값, 대비 8.6:1.
