...
- **검사 445건이 통과한 상태로 배포합니다.** 그중 296건은 진짜 브라우저에서, 10건은 진짜 화면 공유로 돌립니다.
...
통과    검사_브라우저.mjs        231건   32.9초   진짜 브라우저 — 내보내기·보관함·나쁜 상황·규모·기능
통과    검사_촬영.mjs           23건   30.4초   진짜 브라우저 — 자동 촬영 전 과정
통과    검사_인쇄.mjs            8건      3초   진짜 브라우저 — PDF 출력
통과    검사_좁은화면.mjs          6건      4초   창을 실제로 줄여 본다
통과    검사_제목.mjs           18건   16.4초   단계 글 자동 작성 — 화면 글자 읽기
통과    검사_진짜공유.mjs         10건   26.8초   대역 없이 진짜 화면 공유 (창이 잠깐 뜬다)
...
`tests/검사_브라우저_규격.js` 는 화면 값이 규격에서 벗어나지 않았는지 잽니다 — 본문 17px, 행간 1.5, 폭 680px, 대비 8.6:1. 색·간격·모서리는 **디자인 시스템 토큰만** 쓰게 하고 날값을 쓰면 걸러 냅니다.
